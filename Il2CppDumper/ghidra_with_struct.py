...
def set_type(addr, type_str):
	...
	if len(dataTypes) == 0:
		...
		print("Conflicting data types found for type " + type_str + "(parsed as '" + newType + "')")
		return
	else:
		addrType = dataTypes[0]
		
	if addrType is None:
		print("Could not identify type " + type_str + "(parsed as '" + newType + "')")
	else:
	    try:
...
def set_sig(addr, name, sig):
	try: 
		...
		try:
			newSig = sig.replace(", ","ext, ").replace(r"\)","ext\)")
...
with open(f.absolutePath, 'rb') as open_file:
	data = json.loads(open_file.read().decode('utf-8'))
